...
4. 架构复杂性与推理延迟：Encoder-Decoder架构在推理时需要运行两个大型神经网络（先编码整个输入，再解码生成输出），这会增加计算延迟和成本。对于ChatGPT这样的实时对话应用，延迟是致命伤。
...
### 首先是本次的例子：  
`The cat that the boy adopted is sleeping.`（“男孩领养的那只猫正在睡觉。”）  
这个句子的难点：
* “cat” 和 “is sleeping” 距离很远，但语义相关
* “that the boy adopted” 是插入成分，模型必须“跨越”它
* “cat” 是整句的主语
* “boy” 与 “adopted” 关系紧密，但与 “sleeping” 无关
### 我们要模拟什么？
我们模拟模型试图理解句子中某个词，例如我们挑选目标词“sleeping”，模型需要知道“sleeping“的主语是谁？  
也就是，模型应该弄清： “谁在睡觉？” → 其实是 “cat”  
但“cat” 的信息被插入句子中一大段内容隔开，注意力必须跨越插入语。  
这是 Attention 的核心任务：`从整个句子里找出与当前词最相关的部分`。
### 处理流程
#### 词向量构建 
为演示，我们给每个词分配“简单向量”。（真实模型向量是高维，这里只用 2 维展示直觉）  
词	词向量（简单示例）
The	[1, 0]
cat	[2, 1]
that	[1, 1]
...
is	[0, 1]
sleeping	[2, 2]  
这些向量只是为了演示“不同词有不同语义方向”，现实中模型会通过训练自动得到。  
